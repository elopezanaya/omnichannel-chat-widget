...
class ReleaseOrchestrator {
    ...
    async officialReleaseWorkflow(packageInfo) {
        ...
        // Verify that all key files are committed before creating the tag
        await this.verifyKeyFilesCommitted();
        ...
        const tagMessage = await this.prompt(`Enter tag annotation message for mex-${releaseVersion} (no quotes needed):`, `Release version ${releaseVersion}`);        try {
            ...
            // Show tag details and key files for review
            this.printInfo(`Showing contents of tag mex-${releaseVersion} for review:`);
            this.printTagOutputInfo();
            console.log("\n");
            execSync(`git --no-pager show mex-${releaseVersion}`, { stdio: "inherit" });
            console.log("\n");
            
            // Show the key files in the tag (package.json, yarn.lock, CHANGE_LOG.md)
            await this.showKeyFilesInTag(`mex-${releaseVersion}`);
    ...
    async hotfixReleaseWorkflow(packageInfo) {
        ...
        const tagMessage = await this.prompt(`Enter tag annotation message for mex-${confirmedVersion} (no quotes needed):`, `Hotfix version ${confirmedVersion}`);        try {
            ...
            this.printTagOutputInfo();
            console.log("\n");
            execSync(`git --no-pager show mex-${confirmedVersion}`, { stdio: "inherit" });
            console.log("\n");
            
            // Show the key files in the tag (package.json, yarn.lock, CHANGE_LOG.md)
            await this.showKeyFilesInTag(`mex-${confirmedVersion}`);
    ...
    printTagOutputInfo() {
        this.printInfo("Showing tag contents without paging for easier review.");
        this.printInfo("The complete contents will be displayed at once without requiring any key presses.");
        this.printInfo("If you need to scroll up to review earlier content, use your terminal's scrollbar or keyboard shortcuts.");
        this.printInfo("IMPORTANT: You'll see the FULL CONTENTS of key files: package.json, yarn.lock, and CHANGE_LOG.md");
        this.printInfo("Please verify that all these files are present and contain the expected changes before confirming the tag push.");
    }

    async showFileDiffInTag(tagName, filePath) {
        try {
            this.printInfo(`Showing changes to ${filePath} in tag ${tagName}:`);
            console.log("\n");
            execSync(`git --no-pager show ${tagName}:${filePath}`, { stdio: "inherit" });
            console.log("\n");
            return true;
        } catch (error) {
            this.printWarning(`Could not show ${filePath} in tag: ${error.message}`);
            return false;
        }
    }

    async showKeyFilesInTag(tagName) {
        this.printStep(`Reviewing key files in tag ${tagName}`);
        
        // 1. Show package.json
        this.printInfo(`PACKAGE.JSON in tag ${tagName}:`);
        console.log("\n");
        const packageJsonSuccess = await this.showFileDiffInTag(tagName, "package.json");
        if (!packageJsonSuccess) {
            this.printError("CRITICAL: package.json is not in the tag!");
        }
        ...
        // 3. Show CHANGE_LOG.md
        const changelogPath = this.config.changelogPath.replace(/^\.\.\//, "");
        this.printInfo(`CHANGE_LOG.MD in tag ${tagName}:`);
        console.log("\n");
        const changelogSuccess = await this.showFileDiffInTag(tagName, changelogPath);
        if (!changelogSuccess) {
            this.printError("CRITICAL: CHANGE_LOG.md is not in the tag!");
        }
    ...
    async verifyKeyFilesCommitted() {
        this.printStep("Verifying key files are committed");
        
        // 1. Check git status to ensure no uncommitted changes
        const status = execSync("git status --porcelain").toString();
        if (status.trim() !== "") {
            this.printWarning("There are uncommitted changes in your working directory");
            console.log(status);
            
            const forceContinue = await this.promptYesNo("Continue anyway? (Not recommended)");
            if (!forceContinue) {
                throw new Error("Uncommitted changes detected. Please commit them first.");
            }
        }
        
        // 2. Verify package.json is committed
        this.printInfo("Verifying package.json is committed...");
        try {
            execSync("git show HEAD:package.json > /dev/null", { stdio: "ignore" });
        } catch (error) {
            this.printError("package.json is not in the latest commit!");
            const forceContinue = await this.promptYesNo("Continue without package.json in the commit? (Not recommended)");
            if (!forceContinue) {
                throw new Error("package.json must be committed before creating a tag");
            }
        }
        ...
        // 4. Verify CHANGE_LOG.md is committed
        const changelogPath = this.config.changelogPath.replace(/^\.\.\//, "");
        this.printInfo(`Verifying ${changelogPath} is committed...`);
        try {
            execSync(`git show HEAD:${changelogPath} > /dev/null`, { stdio: "ignore" });
        } catch (error) {
            this.printError(`${changelogPath} is not in the latest commit!`);
            const forceContinue = await this.promptYesNo(`Continue without ${changelogPath} in the commit? (Not recommended)`);
            if (!forceContinue) {
                throw new Error("CHANGE_LOG.md must be committed before creating a tag");
            }
        }
        
        this.printSuccess("All key files are committed and ready for tagging");
        return true;
    }
